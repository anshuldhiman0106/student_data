...
export default function StudentDashboard() {
      ...
      (async () => {
        ...
        try {
          ...
          const list = (process.env.NEXT_PUBLIC_ADMIN_EMAILS || "")
            ...
            .filter(Boolean);
          setIsAdmin(
            user && user.email ? list.includes(user.email.toLowerCase()) : false
          );
        } catch (e) {
        ...
        const { data: subscription } = client.auth.onAuthStateChange(
          (event, session) => {
            ...
            setIsAdmin(
              user && user.email
                ? list.includes(user.email.toLowerCase())
                : false
            );
          }
        );
  ...
  const fetchClasses = async () => {
    ...
    setClassList(data.map((x) => x.class));
  };
  ...
  const handleStudentClick = async (student) => {
    try {
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          if (verifyJson.ok) {
            ...
            alert(
              "Payment verification failed: " + (verifyJson.error || "unknown")
            );
          }
        },
        prefill: {
      ...
      if (typeof window !== "undefined" && window.Razorpay) {
        ...
        alert(
          "Razorpay checkout not available. Ensure checkout script is loaded."
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleAuthSubmit = async () => {
    ...
    try {
      if (authMode === "signup") {
        ...
        alert(
          "Sign up successful. Check your email to confirm (if email confirmation is enabled)."
        );
    ...
    return (
      <div className="p-6">
        ...
        <ul className="list-disc pl-5">
          <li>
            <code>NEXT_PUBLIC_SUPABASE_URL</code> — e.g.{" "}
            <code>https://&lt;your-project-ref&gt;.supabase.co</code>
          </li>
          <li>
            <code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> — your public anon key
          </li>
        </ul>
      </div>
    );
          ...
          {authUser ? (
            <div className="text-sm">
              ...
              <div className="mt-2 flex gap-2">
                <button
                  className="px-3 py-1 rounded bg-glass-bg"
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur flex items-center justify-center p-4">
            <div className="bg-black/90 p-6 rounded-lg shadow-md w-full max-w-md">
              ...
              <p className="text-sm mb-4">
                Use one of the providers below to sign in or create an account.
              </p>
              <div className="flex flex-col gap-3">
                <button
                  onClick={() => handleOAuthSignIn("google")}
                  className="w-full px-3 py-2 rounded bg-blue-600/60 text-white"
                >
            ...
            <div className="max-h-64 overflow-auto mb-4">
              {[...selected].slice(0, 50).map((id) => {
                const s = students.find(
                  (x) => String(x.student_id) === String(id)
                );
                if (!s) return null;
                return (
                  <div
